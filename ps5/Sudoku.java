...
public class Sudoku {    
    ...
    // Determines whether it is safe  to place a value in this box
    private boolean isSafe(int val, int row, int col) {
        if (this.rowHasVal[row][val] || this.colHasVal[col][val]) {
            return false;
        }
    ...
    private boolean solveRB(int n) {
        ...
        int row = n % 9;
        int col = n / 9;
        // If we reach the end we have a solution
        if (n == 81) {
            return true;
        }
        // If it's fixed then just move on
        if (this.valIsFixed[row][col]) {
            return this.solveRB(n + 1);
        }
        // If it's not fixed then we have to try each number
        else {
            // Try each number
            for (int i = 1; i < 10; i++) {
                // If that number is safe put it in
                if (this.isSafe(i, row, col)) {
                    this.placeVal(i, row, col);
                    boolean result = this.solveRB(n+1);
                    // If it has a valid solution, return true
                    if (result) {
                        return true;
                    }
                    // If its not valid, get rid of it so we can try the next one
