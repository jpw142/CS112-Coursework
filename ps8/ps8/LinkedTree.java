...
public class LinkedTree {
    ...
    private class PostorderIterator implements LinkedTreeIterator {
        ...
        private Node postorderFirst(Node root) {
            if (root.left != null) {
                return postorderFirst(root.left);
            }
            else if (root.right != null) {
                return postorderFirst(root.right);
            }
            return root;
        }

        private PostorderIterator() {
            nextNode = postorderFirst(root);
        }
        ...
        public int next() {
            ...
            // if next node is the left child of its parent then go to the right
            else if (nextNode == nextNode.parent.left) {
                if (nextNode.parent.right != null) {
                    nextNode = postorderFirst(nextNode.parent.right);
                }
                else {
                    nextNode = nextNode.parent;
                }
            }
            // if it's the right then everything is good and go to the parent
            else {
                nextNode = nextNode.parent;
            }
    ...
    public static void main(String[] args) {
        ...
        tree.insertKeys(new int[]{3, 1, 5, 0, 2, 4, 6});
        ...
        tree2.insertKeys(new int[]{6, 5, 4, 3, 2, 1, 0});
        LinkedTreeIterator iter2 = tree2.postorderIterator();
        while (iter2.hasNext()) {
